...
import { HttpError, HttpErrorCodec, HttpGenericError, httpGenericError, HttpGenericErrorCodec } from './httpErrors';
...
export const okHttpResponsePayload = <TData extends io.Mixed>(data: TData) => io.type({
  protocol: io.literal('http'),
  ok: io.literal(true),
  data,
});
...
export const errHttpResponsePayload = <TErr extends io.Mixed>(error: TErr) => io.type({
  protocol: io.literal('http'),
  ok: io.literal(false),
  error,
});
...
export type ErrHttpResponsePayloadOfType<TErr extends HttpErrorCodec> = GenericErrHttpResponsePayload & {
...
>(ok: TOkCodec, err: TErrCodec) => io.union([ok, err]);
