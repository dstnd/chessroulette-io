...
export const httpInputValidationError = <M extends FormModelCodec>(model: M) => io.type({
  type: io.literal('HttpInputValidationError'),
  invalidInput: getInvalidInputTypeFromModel(model),
});

export const inputValidationError = <M extends FormModelCodec>(model: M) => io.type({
