...
export class AsyncResultWrapper<T, E> {
  ...
  mapErr<E2>(mapper: (val: E) => E2): AsyncResultWrapper<T, E | E2> {
    const mapped: Promise<Result<T, E | E2>> = this.result.then(
...
// export type AsyncResult<T, E> = (AsyncOk<T> | AsyncErr<E>) & AsyncResultWrapper<T, E>;
...
export namespace AsyncResult {
  ...
  export function all<T extends AsyncResult<any, any>[]>(
    ...
    const resolver = Promise
      .all<Result<any, any>>(results.map((r) => r.resolve()))
      .then((results) => Result.all(...results));

      return new AsyncResultWrapper(resolver) as AsyncResult<AsyncResultOkTypes<T>, AsyncResultErrTypes<T>[number]>;
  }
};
